...
InvestmentSchema.index({platform: 1, platformInvestmentId: 1}, {unique: true});

/**
 * Holds user specidic overriders and data pertaining to an investment.
 * Since the investment data is global for all users we want to ensure user changes are kept just for this user.
 */
...
InvestmentUserDataSchema.index({investment: 1, user: 1}, {unique: true});


module.exports = exports = mongoose.alphaflow.model('Investment', InvestmentSchema);
module.exports = exports = mongoose.alphaflow.model('InvestmentUserData', InvestmentUserDataSchema);